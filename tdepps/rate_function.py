...
class NonRateFunction(RateFunction):
    """
    Just does nothing, without breaking the interface scheme.

    If no time is needed, we could just ignore the times, but this would waste a
    lot of time if a real RateFunction would be used.
    So here we just implement all needed methods in a consistent way, but they
    do nothing, which should be as good as skipping them.
    """
    def __init__(self):
        return
    ...
    def fit(self, t, rate, p0=None, rate_std=None, **kwargs):
        """
        We need to overwrite the fit function here, because we shouldn't use
        the minimizer on the dummy function.
        """
        return []
    ...
    def _get_default_seed(self, t, rate, rate_std):
        ...
        -------
        p0 : tuple, shape (0)
            Empty tuple to stay consistent
        """
        return tuple()
